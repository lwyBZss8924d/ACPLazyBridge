...
8.  [Headings](#headings)
    i.  [ATX-style headings](#atx-style-headings)
    ii.  [Use unique, complete names for headings](#use-unique-complete-names-for-headings)
    iii.  [Add spacing to headings](#add-spacing-to-headings)
    iv.  [Use a single H1 heading](#use-a-single-h1-heading)
    v.  [Capitalization of titles and headers](#capitalization-of-titles-and-headers)
9.  [Lists](#lists)
    i.  [Use lazy numbering for long lists](#use-lazy-numbering-for-long-lists)
    ii.  [Nested list spacing](#nested-list-spacing)
10.  [Code](#code)
    i.  [Inline](#inline)
    ii.  [Use code span for escaping](#use-code-span-for-escaping)
    iii.  [Codeblocks](#codeblocks)
        a.  [Declare the language](#declare-the-language)
        b.  [Escape newlines](#escape-newlines)
        c.  [Use fenced code blocks instead of indented code blocks](#use-fenced-code-blocks-instead-of-indented-code-blocks)
        d.  [Nest codeblocks within lists](#nest-codeblocks-within-lists)
11.  [Links](#links)
    i.  [Use explicit paths for links within Markdown](#use-explicit-paths-for-links-within-markdown)
    ii.  [Avoid relative paths unless within the same directory](#avoid-relative-paths-unless-within-the-same-directory)
    iii.  [Use informative Markdown link titles](#use-informative-markdown-link-titles)
    iv.  [Reference links](#reference-links)
        a.  [Use reference links for long links](#use-reference-links-for-long-links)
        b.  [Use reference links to reduce duplication](#use-reference-links-to-reduce-duplication)
        c.  [Define reference links after their first use](#define-reference-links-after-their-first-use)
12.  [Images](#images)
13.  [Tables](#tables)
...
2.  `author`: _Optional_. If you'd like to claim ownership of the document or
    if you are very proud of it, add yourself under the title. However,
    revision history generally suffices.

3.  `Short introduction.` 1–3 sentences providing a high-level overview of the
...
4.  `[TOC]`: if you use hosting that supports table of contents, such as Gitiles,
...
6.  `## See also`: Put miscellaneous links at the bottom for the user who wants
